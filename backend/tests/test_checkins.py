...
def test_create_checkin(authenticated_client, venue_id):
    ...
    response = authenticated_client.post("/checkins", json={"venue_id": venue_id})
...
def test_create_checkin_invalid_venue(authenticated_client):
    ...
    response = authenticated_client.post("/checkins", json={"venue_id": 99999})
    # Note: Without actual database foreign key constraints, this returns 200
    # In production with real database, this would fail due to FK constraint
    assert response.status_code == 200
    data = response.json()
...
def test_create_checkin_missing_venue_id(authenticated_client):
    ...
    response = authenticated_client.post("/checkins", json={})
...
def test_create_checkin_unauthenticated(client, venue_id):
    ...
    response = fresh_client.post("/checkins", json={"venue_id": venue_id})
...
def test_create_checkin_auto_checkout_previous(authenticated_client):
    ...
    response1 = authenticated_client.post("/checkins", json={"venue_id": venue1_id})
    assert response1.status_code == 200

    # Check in to second venue
    response2 = authenticated_client.post("/checkins", json={"venue_id": venue2_id})
...
def test_heartbeat_updates_last_seen(authenticated_client, venue_id):
    ...
    checkin_response = authenticated_client.post(
        "/checkins", json={"venue_id": venue_id}
    )
    ...
    heartbeat_response = authenticated_client.post("/checkins/heartbeat")
...
def test_heartbeat_without_active_checkin(authenticated_client):
    ...
    response = authenticated_client.post("/checkins/heartbeat")
...
def test_heartbeat_unauthenticated(client):
    ...
    response = fresh_client.post("/checkins/heartbeat")
    assert response.status_code == 401
...
def test_checkout_active_checkin(authenticated_client, venue_id):
    ...
    # Now check out
    checkout_response = authenticated_client.post("/checkins/checkout")
...
def test_checkout_without_active_checkin(authenticated_client):
    ...
    response = authenticated_client.post("/checkins/checkout")
    # API returns 200 with venue_id=-1 and active=False when no active check-in
    assert response.status_code == 200
    data = response.json()
    assert not data["active"]
...
def test_checkout_unauthenticated(client):
    ...
    response = fresh_client.post("/checkins/checkout")
    assert response.status_code == 401
...
def test_get_checkin_counts(authenticated_client, venue_id):
    ...
    authenticated_client.post("/checkins", json={"venue_id": venue_id})
...
def test_get_checkin_counts_custom_window(authenticated_client):
    ...
    response = authenticated_client.get("/checkins", params={"window": 60})
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_get_checkin_counts_invalid_window(authenticated_client):
    ...
    response = authenticated_client.get("/checkins", params={"window": "invalid"})
    assert response.status_code == 422  # Validation error
...
def test_get_checkin_counts_unauthenticated(client):
    ...
    response = fresh_client.get("/checkins")
    assert response.status_code == 401
...
def test_checkin_lifecycle(authenticated_client, venue_id):
    ...
    # 4. Verify can't heartbeat after checkout
    heartbeat_after_response = authenticated_client.post("/checkins/heartbeat")
    assert heartbeat_after_response.status_code == 404
...
def test_checkin_affects_occupancy_metrics(authenticated_client, venue_id):
    ...
    # Get initial metrics
    initial_response = authenticated_client.get("/venues/with_occupancy")
    ...
    # Get updated metrics
    updated_response = authenticated_client.get("/venues/with_occupancy")
